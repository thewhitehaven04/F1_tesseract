...
class Enhance_Image():
    ...
    def enhance_image(frame):
        enhanced = cv2.bilateralFilter(frame, 35, 40, 40)
        return enhanced
...
class Parse_Data():
    ...
    def get_list_of_speeds(self):
        for x in range(self.start_frame, self.finish_frame):
            ...
            cropped = Enhance_Image.crop_frame(grayscale, 194, 600, 292, 647)
...
if __name__ == '__main__':
    legreg = Parse_Data('R02_Vettel_Bahrain', 100, 401, fps=50)
    legreg.get_list_of_speeds()
    legreg.output_to_file()
    # speeds = legreg.get_list_of_speeds()
    # legreg.output_to_file(speeds)
    # cv2.
